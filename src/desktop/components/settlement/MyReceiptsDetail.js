...
import MyActuallyMd from './MyActuallyMd';
class MyReceiptsDetail extends React.Component{
    state = {
        expand:false
    }
    constructor(props){
    ...
    componentDidMount(){
        ...
        service.receiptDetail({actualSummaryId:this.props.actualSummaryId}).then((data)=>{
    ...
    res(){
        ...
        service.receiptDetail(recParams).then((data)=>{
            this.setState({dataList:data.rows})
        })
    }
    open(type,record){
    ...
    callback(record){
        ...
        if(record.settleType){
            ...
            if(this.state.record.__status == 'add'){
                ...
                params.receiveCompanyId = datas?datas.receivecompanyid:''
            }else if(this.state.record.__status == 'update'){
                datas = this.state.record;
                params.exchangeRate = record.exchangeRate;
                params.currencyAmount = record.currencyAmount;
                params.receiveCompanyId = datas.receiveCompanyId;
                params.actualId = datas.actualId;
            }
            params.detailType = 'INCOME'
...
export default Form.create()(MyReceiptsDetail)