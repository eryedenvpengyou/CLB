...
import { Form,DatePicker,Select,Button,Table,Input,Dropdown,Menu,Modal,InputNumber} from 'antd';
...
class MyActuallyMd extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            code:{
            ...
            if(val!== 'QDF' && val !== 'SKF'){
                ...
                if( order.value !== ''){
                    let currencyAmount = {
                        ...
                        paymentCompanyId: order.record.paymentcompanyid,
                            ...
                            <div className={style.disableds} >
                                <Form.Item {...formItemLayout} label="结算类型" >
                                    {getFieldDecorator('settleType', {
                                        ...
                                        initialValue: record.settleType || '',
                                        onChange: this.settleChange.bind(this)
                                    })(
                                ...
                                <Form.Item {...formItemLayout} label="费率" >
                                    {getFieldDecorator('rate', {
                                        ...
                                        initialValue: record.rate === 0 ? 0 :
                                            record.rate ? ((record.rate) * 100).toFixed(2) + '%' : ''
                                    })(
                                        <Input readOnly />
                                        )}
                                </Form.Item>
                                <div className={style.message}></div>
                                <Form.Item {...formItemLayout} label="实收金额" >
                                    {getFieldDecorator('actualAmount', {
                                        rules: [
                                            { required: this.state.required, message: '请输入实收金额' }
                                        ],
                                        initialValue: record.actualAmount === 0 ? 0 : (record.actualAmount || ''),
                                    })(
                                        <InputNumber precision={2} disabled={this.state.required ? false : true}
                                            style={{ background: this.state.style2, width:'100%'}}
                                            onBlur={this.amount.bind(this, getFieldValue('actualAmount'), 0)} />
                                        )}
                                </Form.Item>
                                ...
                                <Form.Item {...formItemLayout} label="实收金额（结算币种）" >
                                    {getFieldDecorator('currencyAmount', {
                                        rules: [
                                            { required: true, message: '请输入实收金额（结算币种）' }
                                        ],
...
export default Form.create()(MyActuallyMd)