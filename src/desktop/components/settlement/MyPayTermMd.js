...
class MyPayTermMd extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            code:{
    ...
    render(){
        ...
        const personOptions = this.state.personList.map((item,i)=>{
            return <Select.Option key={item.receiveCompanyId+','+item.receiveCompanyType}>{item.receiveCompanyName}</Select.Option>
        })
        return(
            <div>
                <Modal
                ...
                >
                    <Form>
                        <div style={{ padding: '10px', margin: '15px 25px 10px 25px' }}>
                            <Form.Item {...formItemLayout} label="收款方" >
                                {getFieldDecorator('receiveCompanyName', {
                                    rules: [
                                        { required: true, message: '请选择收款方', whitespace: true }
                                    ]
                                })(
                                    <Select
                                        placeholder='请选择收款方'
...
export default Form.create()(MyPayTermMd)