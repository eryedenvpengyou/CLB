...
class AfterApplicationForm1 extends React.Component {
  ...
  componentWillMount() {
    ...
    service.fetchAfterByAfterId({params}).then((data) => {
      if(data.success){
        this.setState({
          iaLevies:data.rows[0].iaLevies
        })
      }
    });
    ...
    return (
      <Form onSubmit={this.handleSearch}>
        ...
        <Row gutter={40} >
                ...
                <Select  style={{width:'100%',height:'40px',color:'#333'}} disabled = {true}>
                  {
                    ...
                    this.state.codeList.currencyCodes.map((item, index)=>
          ...
          <Col span={10} style={{paddingLeft:'0',paddingRight:'0'}}>
            <FormItem {...formItemLayout} label='主险首期保费' style={{marginBottom:'0'}}>
              {getFieldDecorator('yearPayAmount',{
                initialValue:fields.yearPayAmount || 0
              })(
                <InputNumber
        ...
        <Row gutter={40} >
          <Col span={10} style={{paddingLeft:'0',paddingRight:'0'}}>
            <FormItem {...formItemLayout} label='保额' style={{marginBottom:'0'}}>
              {getFieldDecorator('policyAmount',{
                initialValue:fields.policyAmount || 0
              })(
                <InputNumber
                  style={{width:'100%',color:'#333'}}
                  // formatter={value => value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
          ...
          <Col span={10} style={{paddingLeft:'0',paddingRight:'0'}}>
            <FormItem {...formItemLayout} label='首期保监征费' style={{marginBottom:'0'}}>
              {getFieldDecorator('iaLevies',{
                initialValue: this.state.iaLevies || 0
              })(
                <Input
                  style={{width:'100%',color:'#333'}}
                  disabled = {true}
          ...
          <Col span={10} style={{paddingLeft:'0',paddingRight:'0'}}>
            <FormItem {...formItemLayout} label='首期总保费' style={{marginBottom:'0'}}>
              {getFieldDecorator('firstSumPremium',{
                initialValue:fields.firstSumPremium || 0
              })(
                <InputNumber
                  style={{width:'100%',color:'#333'}}
                  formatter={value => value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                ...
                <Select disabled={true} style={{color:'#333'}}>
                  {
                    ...
                    this.state.codeList.afterProjectCodes.map((item, index)=>
                      <Option value={item.value} key={index}>{item.meaning}</Option>
                    )
                  }
                </Select>
        ...
        <Row gutter={40} >
                ...
                <Select disabled={true} style={{color:'#333'}}>
                  {
                    ...
                    this.props.order.codeList.afterTypeCodes.map((item, index)=>
                      <Option value={item.value} key={index}>{item.meaning}</Option>
                    )
                  }
                </Select>
                ...
                <Select disabled={true} style={{color:'#333'}}>
                  {
                    ...
                    this.state.codeList.asCodes.map((item, index)=>
                      <Option value={item.value} key={index}>{item.meaning}</Option>
                    )
                  }
                </Select>
          ...
          {
            fields.handleEndDate &&
            <Col span={10} style={{paddingLeft:'0',paddingRight:'0'}}>
              <FormItem {...formItemLayout} label='处理截止日期' style={{marginBottom:'0'}}>
                {getFieldDecorator('handleEndDate',{
                  initialValue: fields.handleEndDate.split(' ')[0]
                })(
