...
class OrderYJPerson extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      codeList: {
  ...
  orderList = () => {
    ...
    service.fetchOrderPersonListService(this.state.body).then((data) => {
      if (data.success) {
...
//取消订单
  cancelOrder(orderId){
    Modals.warning(this.orderCancel.bind(this, orderId),"您确定取消订单吗？");
  }
  orderCancel(orderId, flag){
    if(flag){
      let params = {};
    ...
    const columns1 = [
      {
        ...
        render: (text, record, index) => {
          return <div>
            {record.midClass == "FMSZ" &&<a style={{fontSize:'14px',color:'#d1b97f',}} onClick={()=>location.hash = '/order/orderYJ/orderYJDetail/personal/FMSZ/'+record.orderId}>{record.orderNumber}</a>}
            {record.midClass == "GDYL" &&<a style={{fontSize:'14px',color:'#d1b97f',}} onClick={()=>location.hash = '/order/orderYJ/orderYJDetail/personal/GDYL/'+record.orderId}>{record.orderNumber}</a>}
          </div>
        }
      },
      {
      ...
      },
      {
        ...
        render: (text, record, index) => {
          if (text) {
            return <span title="预约/面签时间" style={{fontSize:'14px'}}>{record.reserveDate ? moment(record.reserveDate).format('YYYY-MM-DD HH:mm') : ''}</span>;
          } else {
            return "";
          }
        }
      ...
      },
      {
        ...
        render: (text, record, index) => {
          return <div>
            {
              record.midClass == "FMSZ" &&
              <Dropdown overlay=
                {<Menu>
                  {
                    record.status != 'RESERVE_CANCELLED' && record.status != 'NEW' &&
                    <Menu.Item >
                      <Button type='default' style={{ fontSize: '14px', width: '90px' }}>
                        <Link to={{
                          pathname: `/order/followOrderState/${record.orderId}`,
                        }}>预约跟进</Link>
                      </Button>
                    </Menu.Item>
                  }
                  {
                    record.status != 'RESERVE_CANCELLED' && record.status != 'REFUND' && record.status != 'FINISH' && record.status != 'REFUND_APPLICATION' &&
                    <Menu.Item >
                      <Button type='default' style={{ fontSize: '14px', width: '90px' }} onClick={this.cancelOrder.bind(this, record.orderId)}>取消预约</Button>
                    </Menu.Item>
                  }
                  {
                    (record.status == 'NEED_REVIEW' || record.status == 'NEW' || record.status == 'APPROVING') &&
                    <Menu.Item>
                      <Button type='default' style={{ fontSize: '14px', width: '90px' }} >
                      {
                        <Link to={{
                          pathname: `/production/subscribe/YJ/FMSZ/${record.itemId}`,
                          query: { flag: 'update', orderId: record.orderId, orderNumber: record.orderNumber }
                        }}>修改订单</Link>
                      }
                      </Button>
                    </Menu.Item>
            ...
            }
            {
              ...
              <Dropdown overlay=
                ...
                <Menu>
                  ...
                  { record.status != 'FINISH' && record.status != 'RESERVE_CANCELLED' &&
                    <Menu.Item >
                      <Button type='default' style={{ fontSize: '14px', width: '90px' }} onClick={this.cancelOrder.bind(this, record.orderId)}>取消预约</Button>
                    </Menu.Item>
                  }
                  {
                    ...
                    <Menu.Item>
                      <Button type='default' style={{ fontSize: '14px', width: '90px' }} >
                        {
                        <Link to={{
                          pathname: `/production/subscribe/YJ/GDYL/${record.itemId}`,
                          query: { prePage: 'personal' , flag: 'update', orderId: record.orderId, status: record.status,orderNumber:record.orderNumber }
                        }}>修改订单</Link> 
                        }
                      </Button>
                    </Menu.Item>
...
export default Form.create()(OrderYJPerson);
