...
class OrderImmigrantInvestTeam extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      codeList: {
    ...
    const columns = [
      {
        ...
        render: (text, record, index) => {
          return <div>
            <a style={{fontSize:'14px',color:'#d1b97f',}} onClick={()=>location.hash = '/order/orderImmigrantInvest/OrderImmigrantInvestDetail/team/'+record.orderId}>{record.orderNumber}</a>
          </div>
        }
      },{
        title: '产品信息',
              ...
              <Dropdown  overlay=
                {<Menu>
                  <Menu.Item key={index+'_1'}>
                    <Button type='default' style={{fontSize:'14px',width:'90px'}} onClick={()=>location.hash = '/order/orderImmigrantInvest/OrderImmigrantInvestDetail/team/'+record.orderId}>查看详情</Button>
                  </Menu.Item>
...
OrderImmigrantInvestTeam = Form.create()(OrderImmigrantInvestTeam);

export default OrderImmigrantInvestTeam;
