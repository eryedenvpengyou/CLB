...
import {Table,Menu, Form, Checkbox,Input,Row,Col,Dropdown,Pagination, Button, Select, DatePicker,Tooltip,Icon,InputNumber,Modal,textarea} from 'antd';
...
class OrderSummaryTeam extends React.Component {
  ...
  //查看详情/修改
  orderSelect(record){
    location.hash = `/order/orderDetail/team/${record.orderId}`;
  }
  ...
  handVisble(value){
    if(value.modalValue == 'Y'){
      ...
      //     params.pageSize = this.state.pageSize;
      //     this.props.dispatch({
      //       type: 'order/fetchOrderTeamQuery',
      //       payload: {params},
      //     });
      //   }
      // });
    ...
    const columns = [
      {
        ...
        width : '150px',
        render(text, record) {
          return <span style={{fontSize:'14px'}}>{formatCurrency(record.yearPayAmount)}{record.currency}</span>
        }
      }, {
        title: '渠道',
        ...
        sorter: true,
        render(text, record) {
          return <span  style={{fontSize:'14px'}}>{(record.reserveDate||'').substr(0,16)}</span>
        }
      }, {
        title: '状态',
...
OrderSummaryTeam = Form.create()(OrderSummaryTeam);
export default OrderSummaryTeam;
