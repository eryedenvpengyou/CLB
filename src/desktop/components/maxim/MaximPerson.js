...
class MaximPerson extends Component {
  constructor(props){
    super(props);
    this.state = {
      page: 1,
  ...
  componentWillMount() {
    ...
    service.fetchMaximPersonQuery(params).then((data) => {
      if (data.success) {
        this.setState({
          maximPersonList: data,
          page: 1,
        });
      }
    });
  ...
  render() {
    ...
    const  columns1 = [
      ...
      },
      {
        ...
        width: 110,
        render: (text, record, index) => {
          return <div style={{fontSize:'14px'}}>{record.hkContactPerson}</div>
        }
      }, {
        title: '提交时间',
        ...
        <div className={styles.table_border2}>
          ...
          <div id = 'maximPerson'>
            <Table
              rowKey="key"
              columns={columns1}
              dataSource={this.state.maximPersonList.rows}
              pagination={{
                pageSizeOptions: ['5','10','20','50'],
                pageSize: this.state.pageSize,
                current: this.state.page,
                total:this.state.maximPersonList.total || 0,
              }}
              bordered
            />
          </div>
...
export default Form.create()(MaximPerson);
